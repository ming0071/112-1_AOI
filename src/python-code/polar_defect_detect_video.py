...
def polar_transform_2(circles, frame_copy, roi_scale_factor):
    ...
    for i in range(len(circles)):
        ...
        x2, y2 = min(X + new_R, frame_copy.shape[1]), min(
            Y + new_R, frame_copy.shape[0]
        )
...
def defect_detect(polar_img):
    ...
    for img in polar_img:
        ...
        _, binary_roi = cv2.threshold(
            diff_roi, threshold_value_for_diff, 255, cv2.THRESH_BINARY
        )
        # 添加二值化後的圖像到列表中
    ...
    return total_contour_area_list
        ...
        for i in range(len(sorted_circles) - 1):
            if abs(sorted_circles[i][1] - sorted_circles[i + 1][1]) <= y_threshold:
                ...
                if sorted_circles[i][0] > sorted_circles[i + 1][0]:
                    sorted_circles[i], sorted_circles[i + 1] = (
                        sorted_circles[i + 1],
                        sorted_circles[i],
                    )
                    swapped = True
...
def stable_detected_defect(total_contour_area_list, NG_list):
    # 读取total_contour_area_list
    for i in range(len(total_contour_area_list)):
        if total_contour_area_list[i]:
            NG_list[i] = NG_list[i] - 2
        else:
            NG_list[i] = NG_list[i] + 2
    return NG_list
...
def detect_circles(minDist, param1, param2, threshold_value):
    ...
    _, thresholded = cv2.threshold(
        median_blur_frame, threshold_value, 255, cv2.THRESH_BINARY
    )
    # --------------------------------------------------------
    ...
    if circles is not None:
        ...
        # 极坐标转换
        polar_img = polar_transform_2(
            circles_sorted, frame, (1 + roi_scale_factor / 1000)
        )
        get_total_contour_area_list = defect_detect(polar_img)
        NG_list = stable_detected_defect(get_total_contour_area_list, NG_list)
        for i, (x, y, r) in enumerate(circles_sorted):
            # print(i,x,y)
            # 绘制检测到的圆形
            cv2.circle(frame_copy, (x, y), r, (255, 255, 0), 4)
            cv2.circle(frame_copy, (x, y), 5, (255, 255, 0), -1)
            if NG_list[i] < 0:
                text = {"Good"}
                cv2.putText(
                    frame_copy,
...
cap = cv2.VideoCapture(
    "C://Users//ASUS//Desktop//C++//05_image-process//112-1_AOI//final-project//data//video0105.avi"
)
# # 获取视频的帧率
...
minRadius = 50
maxRadius = 70
roi_scale_factor = 112
threshold_value_for_diff = 80
num_circles = 100  # 假设最多有100个圆形
NG_list = [0] * num_circles  # 初始化所有元素为0
...
cv2.createTrackbar(
    "roi_scale_factor", "Detected Circles", roi_scale_factor, 900, nothing
)
cv2.createTrackbar(
    "threshold_value_for_diff",
    "Detected Circles",
    threshold_value_for_diff,
    255,
    nothing,
...
while True:
    ...
    threshold_value_for_diff = cv2.getTrackbarPos(
        "threshold_value_for_diff", "Detected Circles"
    )
    #  # 每秒检测一次
