...
//                          IuEyeCameraStatus       - specific uEye interface
//
//                          ISpecifyPropertyPages
...
// ============================================================================
/*! \defgroup IuEyeCameraStatus uEye Camera Status Interface
*
*  Proprietary interface for controlling uEye Camera Status features exposed by
*  the capture filter.
*  Allows a DirectShow based program to control and query the camera status feature
*  related parameters that are not accessible via direct show functions.
* \{
*/
// ============================================================================
// {FDB53C15-0E58-4591-9D98-DC081B9631ED}
DEFINE_GUID(IID_IuEyeCameraStatus, 0xfdb53c15, 0xe58, 0x4591, 0x9d, 0x98, 0xdc, 0x8, 0x1b, 0x96, 0x31, 0xed);

interface IuEyeCameraStatus : public IUnknown
{
    /*!
    * \brief   get supported for standby
    * \param   supported   TRUE if supported otherwise FALSE
    *
    * \return  HRESULT     0 on success, error code otherwise.
    */
    STDMETHOD(CameraStatus_IsStandbySupported)(BOOL& supported) = 0;

    /*!
    * \brief   set standby state
    * \param   enabled     enable/disable standby
    *
    * \return  HRESULT     0 on success, error code otherwise.
    */
    STDMETHOD(CameraStatus_SetStandby)(BOOL enabled) = 0;

    /*!
    * \brief   get standby state
    * \param   enabled     status of standby
    *
    * \return  HRESULT     0 on success, error code otherwise.
    */
    STDMETHOD(CameraStatus_GetStandby)(BOOL& enabled) = 0;
};
